...
int main(int argc, char **argv)
{
  ...
  */
  try
  {
    tf_camera_to_camera1.waitForTransform("/camera1_link", "/camera_link", ros::Time(0), ros::Duration(50.0));
  ...
  try
  {
    tf_camera_to_camera1.lookupTransform("/camera1_link", "/camera_link", ros::Time(0), transform_camera_to_world);
  ...
  ros::Duration du(0.01);
       ...
       if(flag1&&flag)
    {
      ...
      cv::imshow("观察视角", depth_im1);
      cv::waitKey(0);
      // cv::imshow("光源视角", depth_im);
      // cv::waitKey(0);
          ...
          for(int j=0;j<800;j++)
        {
          ...
          // std::cout<<"观察相机空间坐标 "<<std::endl;
          // std::cout<<"obj_rotbot_frame x= "<< obj_rotbot_frame.getX()<<std::endl;
          // std::cout<<"obj_rotbot_frame y= "<< obj_rotbot_frame.getY()<<std::endl; 
          // std::cout<<"obj_rotbot_frame z= "<< obj_rotbot_frame.getZ()<<std::endl<<std::endl;


          // std::cout<<"视觉相机的像素点位置是："<<std::endl;
          float new_x=(obj_rotbot_frame.getX())*pixels_permm_x+img_centre_x_;
          // std::cout<<new_x<<std::endl;
          float new_y=obj_rotbot_frame.getY()*pixels_permm_y+img_centre_y_;
          // std::cout<<new_y<<std::endl;

          if(new_x>799||new_x<0||new_y>799||new_y<0){
            // std::cout<<"该点的高度是：  " <<std::endl; 
            // std::cout<<new_x<<std::endl; 
            // std::cout<<new_y<<std::endl; 
            // std::cout<<round(new_x)<<std::endl; 
            // std::cout<<round(new_y)<<std::endl; 
            continue;
          }
         
          if((obj_rotbot_frame.getZ()-depth_im1.at<float>(round(new_x),round(new_y)))>0.0001){
          depth_im1.at<float>(round(new_x),round(new_y))=0;
          // cv::imshow("depthtest", depth_im1);
          // cv::waitKey(0);  
          // std::cout<<"depth_im该点的高度是： = "<<depth_im.at<float>(i,j) <<std::endl;
          // std::cout<<"depth_im1该点的高度是： = "<<depth_im1.at<float>(round(new_x),round(new_x)) <<std::endl;
          // pause();
          }
